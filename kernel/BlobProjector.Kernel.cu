/*  -*- C -*-  */
/* On-the-fly projector based on Blob representation */
/* GPU implementation */

#ifndef _BLOBPROJECTOR_KERNEL_H_
#define _BLOBPROJECTOR_KERNEL_H_
...
__global__ void BlobProjector_Kernel(BlobType blobtype,  /* The profile function identifier */
				     double radius, 
				     double mul, 
				     ...
				     int nbNode,
				     const double *X, /* Input. X is blob coefficient if forward=true, sinogram is forward=false */
				     const bool *MX, /* Mask on X */
				     double *Y, 
				     const bool *MY, /* Mask on Y */
				     bool forward)
{
  ...
  if (Al<sizeDet && Ar>0){ /* Ray must be on detector area */
	...
	if (MY[dpix + pixDet * threadIdx.x])  /* for non masked detector bins */
	  atomicAddDouble(&Y[dpix + pixDet * threadIdx.x],  Tval[dpix-dpixL] * X[blobIdx]);
      }
      else			/* backward */
	val[threadIdx.x] += Tval[dpix-dpixL] * X[dpix + pixDet * threadIdx.x];      
    }
  ...
  /* Other possible reduction algorithm for backward projection (taken from CUDA official documentation) */
