/*  -*- C -*-  */
/* On-the-fly pixel based projector */
/* GPU implementation */

#ifndef _PIXDRVPROJECTOR_KERNEL_H_
#define _PIXDRVPROJECTOR_KERNEL_H_
...
__global__ void PixDrvProjector_Kernel(bool fanbeam, 
				       const int *projset,
				       const double *pSrc, 
				       const double *rSrc, 
				       const double *rDet, 
				       ...
				       bool forward)
{
  ...
  if (forward) {	   /* forward : return immediately if pix coeff is 0 */
    if (fabs(X[pixIdx]) == 0.0)	
      return;
  }
  else {			/* backward : initialize shared memory */
  ...
  double coeff = -1;			 /* intersection length of S-dpix ray and current pixel */
  ...
  if (Al<sizeDet && Ar>0){ /* Ray must be on detector area */
	...
	//if (S.x == A.x || S.y == A.y) {
	coeff = fabs(dot(P-L, dU) - (dpix+0.5)*spDet) < spObj/2 ? spObj : -1;
      }
      else {
	...
	la = fmax(amin, bmin); 
	lb = fmin(amax, bmax);
	coeff = (la < lb) ? (lb - la) * SA : -1;
      }
      if (coeff > 0) {
	//coeff *= spDet;		/* the contribution coefficient is normalized by the size of detector pixel */
	if (forward)
	  atomicAddDouble(&Y[dpix + pixDet * threadIdx.x],  coeff * X[pixIdx]);
	else
	  val[threadIdx.x] += coeff * X[dpix + pixDet * threadIdx.x];
      }
    }
  }
