// TV reconstruction by TVAL3 method and blob representation
...
void save_algo_parms(const string &fname, bool hex, size_t upSpl, 
...
int main(int argc, char **argv) {
  ...
  TCLAP::CmdLine cmd("2D Reconstruction by TVAIADM method. This program reads a directory containning the sinogram and completed acquisition configuration, and the reconstruction results are saved in a sub-directory of the same directory.", ' ', "0.1");
  ...
  TCLAP::ValueArg<double> muArg("m", "mu", "penalty coefficient for data fidelity. [1]", false, 1, "double", cmd);
  TCLAP::ValueArg<double> betaArg("b", "beta", "penalty coefficient for gradient penalty. [1]", false, 1, "double", cmd);
  TCLAP::ValueArg<double> tauArg("T", "tau", "penalty coefficient for proximal penalty. [0.1]", false, 0.1, "double", cmd);
  ...
  // GtG Solver operator
  BlobGtGSolver F(BI, igrid, mu/beta/tau);
  ...
  clock_t t1 = clock();
  SpAlgo::TVIADM(P, G, F, Y, Xr, Nu, Lambda, tveq, aniso, nonneg, mu, beta, tau, tol, maxIter, verbose, 0,
  ...
  save_algo_parms(outpath_fname+"/parameters.cfg", hex, upSpl, 
		  fspls, splStep, frds, radius,
		  tveq, aniso, nonneg, mu, beta, tau, tol, maxIter,
