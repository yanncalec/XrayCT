...
solves the problem (BP) by a 3 steps reweighted l1 iteration, initialized by a reconstructed image (of dimension 512^2) stored in binary file xr.dat.\n");
  ...
  TCLAP::SwitchArg boxSwitch("", "box", "if true the object support is the biggest square included in ROI disk, otherwise it is the smallest square including the ROI disk. [false]", cmd, false);
  ...
  TCLAP::ValueArg<string> algoArg("", "algo","L1-minimization algorithm : padm, fista. [fista]",false,"fista","string", cmd);
  ...
  char buffer[256];
  if(AP.algo=="fista")
    sprintf(buffer, "PixL1_pix[%d]_wvl[%s]_order[%d]_mu[%1.1e]_tol[%1.1e]_rwIter[%d]_rwEpsilon[%2.2f]_algo[%s].%s", AP.pix, AP.wvlname.c_str(), AP.wvlorder, AP.mu, AP.tol, AP.rwIter, AP.rwEpsilon, AP.algo.c_str(), pfix.c_str());
  else
    sprintf(buffer, "PixL1_pix[%d]_wvl[%s]_order[%d]_mu[%1.1e]_beta[%1.1e]_epsilon[%1.1e]_tol[%1.1e]_rwIter[%d]_rwEpsilon[%2.2f]_algo[%s].%s", AP.pix, AP.wvlname.c_str(), AP.wvlorder, AP.mu, AP.beta, AP.epsilon, AP.tol, AP.rwIter, AP.rwEpsilon, AP.algo.c_str(), pfix.c_str());
  ...
  SimConfig conf = CmdTools::load_acqcarm(cfg_fname, AP.pix, AP.roi);
  ...
  if (AP.epsilon<0) {		// Meaningful only when unconstraint BP model is used
    ...
    AP.mu *= mu_factor;
    // Explenation for 1/AP.norm_projecor:
    // min 1/2|c*A_0 f - b|^2 + mu |f|_1 <=> min 1/2|A_0 f - b|^2 + mu/c |f|_1, and f->f/c
  }
  else {
  ...
  for(int n=0; n<AP.rwIter; n++) {
    ...
    if (AP.algo == "padm")
      msg[n] = SpAlgo::L1PADM(*A, W, Y, Cr, AP.mu, AP.epsilon, AP.beta, AP.tau, AP.gamma, AP.tol, AP.maxIter, verbose);
    else if (AP.algo == "fista")
      msg[n] = SpAlgo::L1FISTA(*A, W, Y, Cr, AP.mu, AP.tol, AP.maxIter, 0, 0, verbose);
    else {
      cerr<<"Unknown algorithm: "<<AP.algo<<endl;
      exit(0);
    }
  ...
  //AP.nnz = Tools::l0norm(Cr) *1./ Xr.size(); // This would be better
  Xr = D->forward(Cr);
